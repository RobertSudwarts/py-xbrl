...
def parse_xbrl(
    instance_path: str, cache: HttpCache, instance_url: str or None = None
) -> XbrlInstance:
    """
    ...
    for fact_elem in root:
        ...
        if (
            "context" in concept_name
            or "unit" in concept_name
            or "schemaRef" in concept_name
        ):
            continue
...
def parse_ixbrl_url(
    instance_url: str, cache: HttpCache, encoding: str or None = None
) -> XbrlInstance:
    """
...
def _extract_non_fraction_value(fact_elem: ET.Element) -> float or None or str:
    ...
    try:
        ...
        logger.info(f"float({fact_value}) * pow(10, {value_scale})")
        raise
...
class XbrlParser:
    ...
    def parse_instance(
        self, uri: str, instance_url: str or None = None, encoding: str or None = None
    ) -> XbrlInstance:
        """
